...
fn parse_optifine_html(html: &str) -> Vec<OptifineVersion> {
    ...
    let mirror_url_re = Regex::new(
        r#"http://optifine\.net/adloadx\?f=((?:preview_)?OptiFine_([^_]+)_[^"]+\.jar)"#
    ).unwrap();
    ...
    for cap in mirror_url_re.captures_iter(html) {
        ...
        let filename = if full_filename.starts_with("preview_") {
            full_filename[8..].to_string() // Remove "preview_" prefix
        } else {
            full_filename.clone()
        };
        
        // Avoid duplicates (mirror URLs appear twice - once in ad link, once in mirror link)
        if !versions.iter().any(|v: &OptifineVersion| v.download_url == download_url) {
...
pub async fn fetch_optifine_versions(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| AppError::ApiError(format!("Could not reach OptiFine download servers: {}", e)))?;
...
pub async fn download_optifine(
    ...
    let version = get_optifine_for_mc_version(client, mc_version)
        ...
        .ok_or_else(|| AppError::ContentNotFound(
            format!("No OptiFine available for Minecraft {}", mc_version)
        ))?;
    ...
    let adloadx_response = client
        ...
        .send()
        .await
        .map_err(|e| AppError::DownloadError(format!("Failed to access OptiFine download page: {}", e)))?;
    ...
    let adloadx_html = adloadx_response.text().await
    ...
    let downloadx_path = downloadx_re
        ...
        .map(|m| m.as_str())
        .ok_or_else(|| AppError::DownloadError(
            "Could not find OptiFine download link. The website structure may have changed.".to_string()
        ))?;
    ...
    if !response.status().is_success() {
        return Err(AppError::DownloadError(
            format!("Failed to download OptiFine (HTTP {}). Please try again.", response.status())
        ));
    }
    ...
    if content_type.contains("text/html") {
        return Err(AppError::DownloadError(
            "OptiFine download returned HTML instead of JAR. The download may have been blocked.".to_string()
        ));
    }
