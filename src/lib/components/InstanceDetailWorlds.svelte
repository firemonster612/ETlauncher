...
<div class="flex flex-col h-full">
  <div class="flex items-center justify-between px-6 py-4 border-b border-border">
    <div class="flex items-center gap-3">
      ...
      <div>
        <p class="text-xs uppercase tracking-wide text-muted-foreground">Worlds</p>
          ...
          <div class="border border-border rounded-lg p-3 bg-background/80 flex flex-col gap-3">
            <div class="flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="text-xs text-muted-foreground truncate">Folder: {world.folderName}</p>
                <div class="flex flex-wrap gap-2 mt-1">
                  <span class="text-[11px] px-2 py-0.5 border rounded capitalize">
                    {gameModeLabel(world.gameMode)}
                  </span>
                  {#if world.versionName}
                    <span class="text-[11px] px-2 py-0.5 border rounded text-muted-foreground">
                      {world.versionName}
                    </span>
                  {/if}
                  {#if world.cheatsEnabled}
                    <span class="text-[11px] px-2 py-0.5 border rounded bg-amber-500/20 text-amber-500">
                      Cheats on
                    </span>
                  {/if}
                </div>
            ...
            <div class="flex gap-2">
              ...
              {/if}
              <Button variant="outline" size="sm" onclick={() => openFolder(world)} title="Open folder">
                <FolderOpen class="h-4 w-4" />
              </Button>
              <Button variant="destructive" size="sm" onclick={() => handleDelete(world)} title="Delete world">
