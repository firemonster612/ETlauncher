<script lang="ts">
	...
	const features = [
		...
		},
		{
			icon: Package,
			title: 'Modpacks',
			description: 'Browse and install modpacks from Modrinth, CurseForge, FTB, and Technic',
		},
		{
			icon: Puzzle,
	...
	function handleColorPresetChange(preset: ColorPreset) {
		...
		themeStore.applyAccentColor(preset, preset === 'custom' ? getCustomColors() : undefined);
	}

	function handleCustomColorChange(hue: number, chroma: number) {
		customHue = hue;
		customChroma = Math.max(0.05, chroma * 0.35);
		themeStore.applyAccentColor('custom', { primaryHue: customHue, primaryChroma: customChroma });
	}
	...
	function handleSidebarStyleChange(style: SidebarStyle) {
		...
		themeStore.applySidebarStyle(style, style === 'custom' ? getCustomSidebarColor() : undefined);
	}
	...
	async function completeOnboarding() {
		...
		await settingsStore.update({
			...
			customColors: selectedColorPreset === 'custom' ? getCustomColors() : undefined,
			fontFamily: selectedFont,
			customFont: selectedFont === 'custom' ? { family: customFontFamily } : undefined,
		...
		try {
			...
			const hue = selectedColorPreset === 'custom' ? customHue : presetValues.hue;
			const chroma = selectedColorPreset === 'custom' ? customChroma : presetValues.chroma;
		...
		>
			...
			<div class="flex-1 overflow-y-auto px-6 pb-4">
				...
				{:else if onboardingStore.currentStep === 'login'}
					<!-- Login Step -->
					<div class="animate-in fade-in slide-in-from-right-4 space-y-4 duration-300">
						<div class="text-center">
					...
					<div class="animate-in fade-in slide-in-from-right-4 space-y-6 duration-300">
						...
						<div class="space-y-2">
							...
							<div class="grid grid-cols-4 gap-2 sm:grid-cols-7">
								...
								>
									<Palette
										class="h-6 w-6 {selectedColorPreset === 'custom' ? 'text-primary' : ''}"
									/>
				...
				{:else if onboardingStore.currentStep === 'font'}
					...
					<div class="animate-in fade-in slide-in-from-right-4 space-y-6 duration-300">
						<div class="text-center">
							...
							<p class="text-muted-foreground mt-1 text-sm">Choose your font and sidebar style</p>
						</div>
						...
						<div class="space-y-2">
							...
							<div class="grid grid-cols-3 gap-2">
								...
								>
									<span class="text-lg" style="font-family: 'Silkscreen', monospace">Aa</span>
								...
								>
									<span class="text-lg" style="font-family: system-ui, sans-serif">Aa</span>
									<span class="text-sm">System</span>
								</button>
								<button
									class="flex flex-col items-center gap-2 rounded-lg border-2 p-4 transition-all {selectedFont ===
								...
								>
									<span class="text-lg" style="font-family: {customFontFamily}, sans-serif">Aa</span
									>
						...
						<div class="space-y-2">
							...
							<div class="grid grid-cols-3 gap-2">
								...
								>
									<div class="bg-muted border-border h-6 w-10 rounded border"></div>
								...
								>
									<div class="bg-primary/20 border-primary/40 h-6 w-10 rounded border"></div>
				...
				{:else if onboardingStore.currentStep === 'curseforge'}
					...
					<div class="animate-in fade-in slide-in-from-right-4 space-y-6 duration-300">
						...
						<div class="space-y-4">
							<div class="space-y-2">
								<label for="curseforge-api-key" class="text-sm font-medium">API Key</label>
								<div class="flex gap-2">
									<input
							...
							<div class="bg-muted/50 space-y-3 rounded-lg p-4">
								<p class="text-sm">
									A CurseForge API key is required to browse and download content from CurseForge.
								</p>
								<p class="text-muted-foreground text-sm">
									A built-in API key is planned for a future release. For now, you'll need to get
									your own free key from CurseForge.
								</p>
								<Button variant="link" onclick={openCurseForgeConsole} class="w-full">
									<ExternalLink class="mr-2 h-4 w-4" />
									Get Free API Key
								</Button>
							</div>
							...
							<div class="mt-4 flex items-center justify-between">
								...
								<div class="flex gap-1.5">
									{#each features as feature, i (feature.title)}
										<button
											class="h-2 w-2 rounded-full transition-all {i === currentFeatureIndex
						...
						<div class="bg-muted/50 space-y-3 rounded-lg p-4">
							<div class="flex items-center justify-between text-sm">
								...
								<span class="font-medium">
									{#if accountsStore.accounts.length > 0}
										{accountsStore.activeAccount?.username || accountsStore.accounts[0]?.username}
									{:else}
										Not signed in
									{/if}
								</span>
							...
							<div class="flex items-center justify-between text-sm">
								...
								<span class="font-medium capitalize">{selectedSidebarStyle}</span>
							</div>
							<div class="flex items-center justify-between text-sm">
			...
			<div class="border-border flex items-center justify-between border-t px-6 py-4">
				...
				{:else}
					...
					<Button size="sm" onclick={handleNext}>
						...
						{:else}
							{onboardingStore.currentStep === 'welcome' ? 'Get Started' : 'Next'}
							{#if !onboardingStore.isLastStep}
								<ChevronRight class="ml-1 h-4 w-4" />
							{/if}
						{/if}
