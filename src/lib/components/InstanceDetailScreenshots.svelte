...
<div class="flex h-full flex-col">
	<div class="border-border flex items-center justify-between border-b px-6 py-4">
		<div class="flex items-center gap-3">
			...
			<div>
				<p class="text-muted-foreground text-xs tracking-wide uppercase">Screenshots</p>
		...
		{:else}
			<div class="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
				{#each filteredScreenshots as shot, index (shot.filename)}
					<button
						class="border-border bg-muted/30 group relative aspect-[4/3] overflow-hidden rounded border"
					...
					>
						...
						<div
							class="absolute right-0 bottom-0 left-0 flex items-center justify-between bg-black/50 px-2 py-1 text-xs text-white"
						>
