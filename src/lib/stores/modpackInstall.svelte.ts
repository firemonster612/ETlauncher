...
function createModpackInstallStore() {
	...
	return {
		...
		async init() {
			...
			try {
				const status = await invoke<ModpackInstallStarted | null>('get_modpack_install_status');
				if (status) {
					isInstalling = true;
					modpackName = status.modpackName;
				}
			} catch (e) {
				console.error('[modpackInstallStore] Failed to get initial status:', e);
			}

			// Listen for install started
			unlistenStarted = await listen<ModpackInstallStarted>('modpack_install_started', (event) => {
