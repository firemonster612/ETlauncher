...
const TUTORIAL_STEPS: TutorialStep[] = [
  ...
  // Step 2-4: Modpacks flow
  {
    ...
    description: "The easiest way to start! Browse thousands of modpacks from Modrinth and CurseForge",
    position: "right",
  ...
  },
  {
    ...
    description: "Click any modpack to see details, then click Install to create a new instance with all mods included",
    position: "bottom",
  },
  // Step 5-7: Custom instance flow
  {
    ...
    description: "Now let's see how to create a custom instance. This is where all your Minecraft installations live",
    position: "right",
  },
  {
    id: "new-instance-btn",
  ...
  },
  {
    ...
    description: "Choose your Minecraft version and loader (Fabric, Forge, etc.) before creating the instance",
    position: "bottom",
    onEnter: () => {
      const newButton = document.querySelector("[data-tutorial='new-instance-btn']") as HTMLButtonElement | null;
      newButton?.click();
    },
  },
  {
    ...
    navigateTo: "/instances",
    onEnter: async () => {
      const createCancel = document.querySelector("[data-tutorial='create-cancel']") as HTMLButtonElement | null;
      createCancel?.click();
  ...
  },
  {
    ...
    description: "Use Check for Updates to bump Minecraft versions or reinstall loaders for this instance",
    position: "top",
    navigateTo: "/instances",
    onEnter: async () => {
      const createCancel = document.querySelector("[data-tutorial='create-cancel']") as HTMLButtonElement | null;
      ...
      const updateButton = (await waitForSelector("[data-tutorial='instance-update-button']", 20, 150)) as HTMLElement | null;
      const sheetContent = document.querySelector("[data-slot='sheet-content']") as HTMLElement | null;
      if (updateButton && sheetContent) {
        const buttonTop =
          updateButton.getBoundingClientRect().top - sheetContent.getBoundingClientRect().top + sheetContent.scrollTop;
  ...
  },
  {
    ...
    navigateTo: "/instances",
    onEnter: async () => {
      const settingsClose = document.querySelector("[data-slot='sheet-close']") as HTMLButtonElement | null;
      settingsClose?.click();
  ...
  },
  {
    ...
    title: "Content Types",
    description: "Choose from mods, shaders, or resource packs - search online and install directly to your instance",
    position: "bottom",
    onEnter: () => {
      const contentButton = document.querySelector("[data-tutorial='content-browser-btn']") as HTMLButtonElement | null;
      contentButton?.click();
    },
  },
  // Step 10: Completion
  {
    ...
    description: "Your instances appear here. Click the play button to launch, update versions, or customize with content!",
    position: "right",
  },
];
...
const waitForSelector = async (selector: string, attempts = 30, delayMs = 200): Promise<HTMLElement | null> => {
...
const ensureTutorialInstance = async () => {
  const existing = document.querySelector("[data-tutorial='instance-settings-btn']") as HTMLElement | null;
  ...
  const createButton = (await waitForSelector("[data-tutorial='instance-create']")) as HTMLButtonElement | null;
  createButton?.click();
